...
import ForegroundTapModal, {
  ArxSignMessageResult,
} from "@/components/modals/ForegroundTapModal";
import { getAuthToken } from "@/lib/client/localStorage";

export default function RegisterSigCard() {
  const router = useRouter();
  const [image, setImage] = useState<string>("");
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const [signaturePublicKey, setSignaturePublicKey] = useState<string>();

  useRequireAdmin();

  const onTap = async (args: ArxSignMessageResult): Promise<void> => {
    ...
    const response = await fetch("/api/admin/sig_card/initialize", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        signaturePublicKey: args.pubKey,
      }),
    });
    ...
    setSignaturePublicKey(args.pubKey);
  };
  ...
  const getImageUrlMutation = useMutation({
    ...
    mutationFn: async ({
      ...
      const newBlob = await upload(imageFile.name, imageFile, {
        access: "public",
        handleUploadUrl: `/api/admin/sig_card/upload?token=${token}`,
      });
      return newBlob.url;
    },
  });
  ...
  const onSubmit = async (formValues: RegisterLocationType) => {
    ...
    await getImageUrlMutation.mutateAsync(
      { token: authToken.value, imageFile },
      {
        onSuccess: async (imageUrl: string) => {
          ...
          const response = await fetch("/api/admin/sig_card/register", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
  ...
  if (!signaturePublicKey) {
    return (
      <ForegroundTapModal
        message="1234" // dummy message
        onTap={onTap}
        onClose={() => {
          toast.info("Sig card registration cancelled");
          router.push("/admin");
        }}
      />
    );
...
RegisterSigCard.getInitialProps = () => {
